...
impl CBackend {
    ...
    fn finish(&mut self) -> String {
        ...
        out.push_str("}\n\n");
        // The one entry point a host that loads this translation unit as a
        // library has to call before anything else in it runs. It does what
        // `main` does before the program's first statement, which today is
        // interning the string literals — a global read before it happens is
        // a null pointer, and the first thing that dereferences one ends the
        // process.
        //
        // Exported and named on purpose. `keal_init_literals` is static and
        // its name is this file's business; a host that reaches for it is
        // holding something that may be renamed. This one will not be.
        //
        // Idempotent, so a host that cannot tell whether it has already run
        // may call it again. Not thread-safe, and does not need to be: it
        // belongs at load time, before anything else has started.
        out.push_str(
