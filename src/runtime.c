...
KEAL_FN KealList* keal_run_command(KealList* argv) {
    ...
    /* The command line, as wide characters. The program's path is token
     * zero even though it is also passed as lpApplicationName: the first
     * token is consumed as argv[0] regardless, so leaving it out loses an
     * argument. */
    KealBuf line;
    keal_buf_init(&line);
    wchar_t* app = NULL;
